...
WHERE schemaname NOT IN ('pg_catalog', 'information_schema', 'auth', 'realtime', 'pgsodium', 'supabase_migrations', 'vault', 'storage')
...
const INDEX_SIZES_QUERY = `SELECT c.relname AS name,
...
const TABLE_INDEX_SIZES_QUERY = `SELECT c.relname AS table,
...
const TABLE_SIZES_QUERY = `SELECT c.relname AS name,
...
const TOTAL_INDEX_SIZES_QUERY = `SELECT pg_size_pretty(sum(c.relpages::bigint*8192)::bigint) AS size
FROM pg_class c
LEFT JOIN pg_namespace n ON (n.oid = c.relnamespace)
WHERE n.nspname NOT IN ('pg_catalog', 'information_schema', 'auth', 'realtime', 'pgsodium', 'supabase_migrations', 'vault', 'storage')
AND n.nspname !~ '^pg_toast'
...
const TOTAL_TABLE_SIZES_QUERY = `SELECT c.relname AS name,
...
WHERE NOT indisunique AND idx_scan < 50 AND pg_relation_size(relid) > 5 * 8192 AND schemaname NOT IN ('pg_catalog', 'information_schema', 'auth', 'realtime', 'pgsodium', 'supabase_migrations', 'vault', 'storage')
