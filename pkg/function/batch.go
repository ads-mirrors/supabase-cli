...
func (s *EdgeRuntimeAPI) UpsertFunctions(ctx context.Context, functionConfig config.FunctionConfig, filter ...func(string) bool) error {
	...
	for slug, function := range functionConfig {
		...
		fmt.Fprintf(os.Stderr, "Deploying Function %s (script size: %s)", slug, functionSize)
...
func toFileURL(fp string) *string {
	if len(fp) == 0 {
		return nil
	}
	parsed := url.URL{Scheme: "file"}
	if filepath.IsAbs(fp) {
		prefix := filepath.VolumeName(fp)
		fp = strings.TrimPrefix(fp, prefix)
	} else {
		parsed.Path = "/"
	}
	parsed.Path += filepath.ToSlash(fp)
